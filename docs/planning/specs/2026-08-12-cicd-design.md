...
Fix: freeze the current local environment (`pip freeze`) to `requirements-lock.txt` at the repo root. This is the exact dependency closure already proven by 101 passing tests across sub-projects 1-5. CI and Docker both install from this file — no fresh resolution, ever.
